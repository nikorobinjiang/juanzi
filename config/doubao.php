...
return [
    ...
    */
    'booking' => [
        ...
        | 场地空闲查询
        |
        | 开发区有 1 号、2 号两个场地，每个都能拼场（1 → 1A/1B，2 → 2A/2B）：
        | - 整场 1/2 = "不拼"，整场被占用时两个半场都不可约；反之任一半场被占用，整场也不可约
        | - 1A 与 1B 之间可互拼（同一时段各占一个半场）
        | - 派生：整场 1 可约 ⟺ 1A 空闲 且 1B 空闲
        */
        'availability' => [
            // 小于该长度的碎片空档不展示（分钟），避免出现 "10:20-10:35" 这种没法约的零星空档
            'min_free_minutes' => 30,
            // 开发区可约单元（数组顺序即展示顺序）
            'development_units' => ['1', '1A', '1B', '2', '2A', '2B'],
        ],
    ],
